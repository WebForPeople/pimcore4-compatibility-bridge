...
class Targeting extends \Zend_Controller_Plugin_Abstract
{
    ...
    {
        if (!Tool\Legacy::useFrontendOutputFilters($request)) {
            return $this->disable();
        }

        $db = \Pimcore\Db::get();
        $enabled = $db->fetchOne("SELECT id FROM targeting_target_groups UNION SELECT id FROM targeting_rules LIMIT 1");
        if (!$enabled) {
            return $this->disable();
        }
